...
static struct animal *get_animal_specs(char *str, int lineno)
{
    ...
    r = sscanf(str, "( %d %d) %d %d | %d %d %d %d %d %d %d %d |", &i, &j, &d, &e, &genes[0], &genes[1], &genes[2], &genes[3], &genes[4], &genes[5], &genes[6], &genes[7]);
    if (r == 12)
    {
...
static char *fetch_line_aux(char *buf, int buflen, FILE *stream, int *lineno)
{
    ...
    if (s == NULL)
        fprintf(stderr, "stdin:line %f: premature end of input\n", *lineno);
    return s;
}
...
int read_wdf(struct world *world)
{
    ...
    int result = (s = fetch_line_aux(buf, BUFLEN, stdin, &lineno)) && get_world_dimens(world, s, lineno) &&
                 (s = fetch_line_aux(buf, BUFLEN, stdin, &lineno)) && get_eden_dimens(world, s, lineno) &&
                 (s = fetch_line_aux(buf, BUFLEN, stdin, &lineno)) && get_plant_energy(world, s, lineno) &&
                 (s = fetch_line_aux(buf, BUFLEN, stdin, &lineno)) && get_reproduction_threshold(world, s, lineno);
